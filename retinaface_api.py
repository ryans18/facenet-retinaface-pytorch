...
#   一定要注意重新编码人脸。
#--------------------------------------#
class RetinafaceApi(object):
    _defaults = {
        "face_path": 'face/face.jpg',
        "face_model":"model_data/face.json",
        #----------------------------------------------------------------------#
        #   retinaface训练完的权值路径
        #----------------------------------------------------------------------#
    ...
    def load_face(self):
        try:
            self.known_face_encodings = np.load("model_data/{backbone}_face_encoding_api.npy".format(backbone=self.facenet_backbone))
            self.known_face_names     = np.load("model_data/{backbone}_names_api.npy".format(backbone=self.facenet_backbone))
        except:
    ...
    def encode_face_dataset(self):
        #---------------------------------------------------#
            #   打开人脸图片
            #---------------------------------------------------#
            image       = np.array(Image.open(self.face_path), np.float32)
            #---------------------------------------------------#
            #   对输入图像进行一个备份
            #---------------------------------------------------#
            ...
            np.save("model_data/{backbone}_face_encoding_api.npy".format(backbone=self.facenet_backbone),face_encoding)
    #---------------------------------------------------#
    #   检测图片得到预测名称
    #---------------------------------------------------#
    def detect_image_2_name(self, image):
        #---------------------------------------------------#
        #   对输入图像进行一个备份，后面用于绘图
        #---------------------------------------------------#
        old_image   = image.copy()
        ...
        #-----------------------------------------------------#
        # 计算人脸距离
        dis = np.linalg.norm(self.known_face_encodings-face_encoding, axis=0)
        if dis <= self.facenet_threhold: return True
        else: return False
        #-----------------------------------------------#
        #   人脸特征比对-结束
        #-----------------------------------------------#
